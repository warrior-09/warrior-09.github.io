...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Orangeの小屋🍊")[0];
}
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Orangeの小屋🍊") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Orangeの小屋🍊的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
